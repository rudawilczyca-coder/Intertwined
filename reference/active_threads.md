# ACTIVE THREADS / LOADED GUNS

**As of:** Wednesday, 7 August 2002, dawn — Paris / Theo's location unknown.
**Last fixed beat:** Draco wakes to an empty bed and finds Jackie doing push-ups in the cleared main room after the January 6 nightmare returned.
**Current arc source:** `arcs/the_honeymoon_they_owed.md`.

This is the single live-thread ledger. Each item records only current status, the unresolved edge, and its source. Event recap belongs in the cited source; present emotional state belongs in `reference/current_DracoTheo_state.md`.

**Boundary:** Ruby and the Blackthorn pack remain off-page under absolute no-contact. Their continuing lives are not active hooks and must not be used to engineer a reunion.

---

## CURRENT ARC

### 🔴 Paris → Fontainebleau, Aug 6–10
Tuesday is played through Rue Chapon. Wednesday opens at dawn in the Pissarro Suite; near midday begins with an ordinary Muggle catacombs group before a specialist guide takes the wizards into the cursed/magical area; Luc's second sitting is fixed for 20:00. Still ahead: Jackie's proposed Samaya meeting Thursday; Fontainebleau and the World Cup Final Saturday.
**Unresolved:** everything after Wednesday's opening beat.
**Source:** `arcs/the_honeymoon_they_owed.md`.

### 🔴 Viola's audience / False Eli
Theo was taken from Haven House during the night of Aug 6–7. The exact taking remains reserved for later play. Viola's intended audience / False-Eli operation is now in motion; Leandro Velluti is still intended to reach Fontainebleau wearing Eli Lavigne's face.
**Unresolved:** the taking scene, Theo's terms at court, Leandro's briefing and behaviour, and the reveal mechanism.
**Source:** `characters/London_Camarilla.md` → "Viola's Interest in the France–Wales Final" and "The False-Eli Operation".

### 🔴 Eli cover against the Fontainebleau perimeter
Theo's cover is fixed: prepared Polyjuice and Eli's supplied hair are together in the office desk's second drawer. Gringotts raises four rings Thursday midnight; anything crossing the third ring is read.
**Unresolved:** how a legally dead vampire crosses under an identity Genevieve already recognised at the April wedding.
**Source:** `arcs/the_honeymoon_they_owed.md`; France 1.0 in `events/book4_post_florence_2001.md`.

### 🟠 Children
Draco has told Jackie he wants children with her for himself; Jackie has said she wants to carry his child one day. Theo has not heard this. Narcissa's three-year heirs clock continues underneath.
**Unresolved:** timing, Theo's place, legal names and inheritance, and reproductive mechanics.
**Source:** `arcs/the_honeymoon_they_owed.md`; `events/book8_liverpool_feb-apr2002.md` → Feb 27.

### 🟠 Luc Moreau's second sitting
Luc is due Wednesday evening to draw Draco nude for a portrait to be hidden until Theo's November birthday. Draco's consent and reproduction terms are fixed.
**Unresolved:** the sitting and Theo's eventual reaction.
**Source:** `arcs/the_honeymoon_they_owed.md`.

### 🟠 Sylvie Roux and the three books
Jackie intends to send three old books to Sylvie through the Lavigne raven network despite Vasseur's warning that the route itself will be noticed.
**Unresolved:** the books' identities, who notices, and the consequences.
**Source:** `arcs/the_honeymoon_they_owed.md`.

---

## PROMISES, SECRETS, AND KNOWLEDGE EDGES

| Thread | Current edge | Source |
|---|---|---|
| **Theo's wedding repair** | Chosen vows, a real honeymoon, and time belonging to his marriage; endorsed but unscheduled. | Book Nine, Phase Five |
| **Draco's apology letter** | Promised before the end of July; no record of writing or sending. | `sessions/july6_karaoke_day.md` |
| **Genevieve's third ticket** | She counted a legally dead man in writing; nobody knows what she wants. | Book Nine, Phase Five |
| **Jackie's private exit arithmetic** | Alice's to reveal. Theo and Draco must not learn it by inference. | `sessions/primrose_hill_walk_handoff.md` |
| **The "Yours" parallel** | Only Draco knows he used the same word with Jackie and Ruby. Dormant. | `sessions/primrose_hill_walk_handoff.md` |
| **The Swansea attic book** | Sylvie's warm-paper test has re-armed Draco's suspicion; he has told nobody. Distinct from the US-trip soul-bond grimoire. | `arcs/the_honeymoon_they_owed.md`; `characters/ruby_vex_character_sheet.md` |
| **Charlie–Draco doorway conversation** | Reserved, unwritten, and unrevealed to reader, Jackie, and Theo. | `sessions/the_flat_july11_played_handoff.md` |

---

## EXTERNAL AND SYSTEMIC PRESSURES

| Thread | Current edge | Source |
|---|---|---|
| **Theo's London standing** | Unpresented and unreleased in Viola's Domain; Filippo remains liable under the Accounting; three Primogen debts remain banked. | `characters/London_Camarilla.md` |
| **Dante** | Three Liverpool manifests connect him to the legal architecture; not yet actionable. | `events/book8_liverpool_feb-apr2002.md`; `characters/Dante_Character.md` |
| **Voss / Aldous Greaves** | Alive in Camarilla custody; identity remains a working assumption, not confirmed fact. | `events/book8_liverpool_feb-apr2002.md` |
| **Drake Lightwood** | Albion Maritime leverage exists and has never been activated. | `events/book8_liverpool_feb-apr2002.md`; `characters/auror_trainees.md` |
| **Soul bond** | Theo's direction transferred to the Malice when he died and lay dormant. During the Aug 6–7 taking, the Malice reached Jackie through it for the first time; sleeping Jackie translated the unfamiliar contact into the January 6 nightmare and does not consciously know its source. | `arcs/the_honeymoon_they_owed.md`; `lore/canon_index.md`; `characters/London_Camarilla.md` |
| **Theo's Malice continuity** | He experienced the Liverpool Malice run without blackout; meaning still unknown. | `events/book8_liverpool_feb-apr2002.md` |

---

The heat is in the unresolved edge. For exact dialogue, sequence, or evidence, open the cited source rather than expanding this ledger.
